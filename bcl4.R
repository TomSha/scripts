...
EpochCorr <- function(prefix){

	prefix<-prefix_list[prefix];
	binary<-paste("/media/meyer-lab/Elements/Work/Stimulus_Barrage/barrage_v2/H2Bslow/",prefix,"/im/im_slice_1/results_toolbox/",prefix,".bin",sep="");
	...
	matep<-matrix(floor(20*(ep$V1+90)),ncol=2,byrow=T);
	nEP=nrow(ep)/2;
	corvec<-rep(0,ncells);
	stim<-rep(-1,nz);
...
EpochCorr2 <- function(prefix){
	    ...
	    corvec<-rep(0,ncells);
		stim<-rep(0,nz);
		stim[1:(matep[1,1]-1)]<-1
		stim[matep[nEP,2]:nz]<-1

		cutstim<-rep(-1,sum(stim==1))
		veclen<-length(cutstim)
		ss<-rep(0,veclen)

		for(i in 1:ncells){
			trace<-as.numeric(allresp[i,])
			bb<-bcl(trace/mean(trace),0.01,0.8,9e-3,1e-3,.4,0,20,"EXP");
			ss[1:(matep[1,1]-1)]<-bb[[3]][1:(matep[1,1]-1)]
			ss[matep[1,1]:veclen]<-bb[[3]][matep[nEP,2]:nz]
			ss[ss==0]=-1;
			corvec[i]=sum(ss*cutstim)/length(cutstim)
...
plot_cell<-function(id,prefix){
	...
	system(paste(bin,"/read"," ",id," ",folder,"\n",sep=""));
