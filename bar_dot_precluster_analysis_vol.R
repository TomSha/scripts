...
#look at selectivity comparing different size dots. Compare 5 vs 17,26 and 30 degrees. Compare 10 vs 17,26,30 degrees - this is useless, cluster on sizes instead
...
#calculate epoch SD, mean and coefficient of variation
...
outputSM_list<-lapply(outputS_list,function(x) t(apply(x,1, function(x) x/mean(x))))
sliceMean<-t(sapply(outputSM_list,colMeans))
sliceSD<-t(sapply(outputSM_list,function(x) apply(x,2,sd)))
...
ResponseRatio<-vector("list",length(outputSM_list))
for (i in 1:length(outputSM_list)) ResponseRatio[[i]]<-outputSM_list[[i]][,1:(nEP/2)*2]/outputSM_list[[i]][,1:(nEP/2)*2-1]
...
prob_dist_list<-lapply(Olist,function(output) t(apply(output[,3:ncol(output)],1,function(x) x/sum(x))))
entropy_list<-lapply(prob_dist_list,function(prob_dist) t(apply(prob_dist,1,function(x) sum(x*log2(1/x)))))
num_SF<-ncol(Olist[[1]][,3:ncol(Olist[[1]])])
equal_prob<-1/num_SF
max_ent<-log2(num_SF)

#calculate the entropy of all the cells over all stimuli
prob_dist_all<-t(apply(outputRCT[,3:ncol(outputRCT)],1,function(x) x/sum(x)))
entropy_all<-t(apply(prob_dist_all,1,function(x) sum(x*log2(1/x))))
max_ent_all<-nEP*((1/nEP)*log2(nEP))

#calculates the min and max value in a list and normalises the list to those values
min_max_list<-function(input_list){
	min_val<-min(sapply(input_list,function(x) min(x)))
	max_val<-max(sapply(input_list,function(x) max(x)))
	lapply(input_list,function(x) ((x)-min_val)/(max_val-min_val))
}
...
#11 Plot_entropy()			Plots histogram and boxplot of cells' entropy over different sizes
#12 Plot_entropy_scatter()	Plots scatter plots of entropies over different sizes 
...
#11 Plots the entropy of all cells given a direction and shape (bar/dot)
plot_entropy<-function(){
	par(mfrow=c(2,2))
	min_break<-min(sapply(entropy_list,min))
	max_break<-max(sapply(entropy_list,max))
	
	for (i in 1:length(entropy_list)){
		hist(entropy_list[[i]][1,],xlim=c(0,2.5),ylim=c(0,13),breaks=seq(min_break,max_break,l=150),freq=F,main=paste("entropy of response to different sizes of",EPsplit[i]),xlab="",ylab="entropy")
		abline(v=max_ent,lty=3,col=rgb(0,0,1))
	}

	X11(); 
	increment<-1/length(entropy_list)
	wex<-increment-(increment*0.1)
	location<-0
	boxplot(x=entropy_list[[1]][1,],boxfill=rgb(1, 1, 1, alpha=1), border=rgb(1, 1, 1, alpha=1),cex.axis=0.8,las=2,ylim=c(0,max_ent),ylab="entropy")
	abline(h=max_ent,lty=3)
	for (i in 1:length(entropy_list)){
		boxplot_params<-boxplot(x=entropy_list[[i]][1,],add=T,boxwex=wex,at=0.60+location,yaxt="n",boxfill=martincolorscale[i])
		location<-location+increment
	}
	legend("bottomleft",inset=0.05,legend=EPsplit,fill=martincolorscale[1:4])
}

#12	Plots scatter plots of entropies over different sizes 
plot_entropy_scatter<-function(){
	pairs(t(do.call(rbind,entropy_list)),labels=EPsplit,upper.panel=NULL)
	mtext("scatter plot of entropy")
}




plot_slice_resp<-function(P=19,C=1.5){
	#image(1:nx,1:ny,matrix(avtab$V2,ncol=ny,nrow=nx),ylim=c(ny+0.5,0.5),col=gray.colors(100),xaxt='n',yaxt='n',ann=FALSE)
	#points(x,y,col=rgb(1,0,0,outputnorm[,epoch]),pch=P,cex=C)
	if((numSlices %% 2)==1){
		mymat<-numSlices+1
	...
	for (i in 1:numSlices){
		...
		y<-cenfile_list[[i]]$V2
		Yflip<-rev(range(y))
		plot(x,y,col=rgb(0,0,0,rowMeans(min_max_list(outputS_list)[[i]])),pch=P,cex=C,ann=T,xaxt="n",yaxt="n",bty="n",main=paste("slice",i+minSlice-1,sep=""),xlab="",ylab="",ylim=Yflip)
	}
	mtext("response heatmap of slices",outer=T)
}
